...
**Current milestone:** Phase 4 (glassOS & the premium pass) is **complete and deployed** (gh-pages; deploy with `scripts/deploy.sh` — Actions is deliberately manual-only on the free tier, see `.github/workflows/ci.yml`). Shipped: glassOS runtime, 3 cases, 4 anthology anchors, Learn shelf, hero perf, 7 diegetic artifacts, Kitbitz debrief vignettes. **The video register pilot:** `dr-anita-still.mp4` (Veo i2v via the user's Google AI Pro, 10s/720p/9:16, 2.3MB) is committed, synced, and wired; runtime support (artifactVideo → real `<video>`, stills as posters, `OSTell.at` timestamps, videos out of PWA precache) shipped. **Pilot verified end-to-end (2026-09-11):** `node scripts/anita-video-shot.mjs` against `npm run preview` — or `SHOT_BASE=https://sammatuba.github.io/blackglass` against the deployed site, falling in through the hub where the host has no SPA fallback — drives Maya→Bea→council→For You, opens the clip (Bea's `bea-won` chain is gated on `inspected: 'anita'` — the video is required, not raced), asserts `<video src*=dr-anita>` + poster, and rides the coda out to the rack. **Full-game push (2026-09-11):** `docs/GAME-PLAN.md` defines outcomes, targets, milestones M1–M7, and the external-asset placeholder register. Shipped in this push: **continuity** (live glassOS runs persist per phone — “Continue” on the rack; verified by `scripts/resume-smoke.mjs`), the **case board** (evidence/inspected stored per lived run, clues surfaced on the rack and anchor select), and **THE CONVERGENCE** (the global finale, unlocked by living all 10 phones, assembled from the player’s recorded choices with a signal report; verified by `scripts/finale-smoke.mjs`). **Next:** the user's review of the 2026-09-13 design pass on the live site, then the Tito Mike clip (`deepfake.ts`, add `at` tells once it lands), M6 content breadth, and whatever the review surfaces. **Human cadence (2026-09-12):** the rule pump now types like a person — typing time is derived per message from its length, a burst shows one indicator and its lines land seconds apart, prose gets reading air, and a single serialized pump keeps two storylines from interleaving; Settings → Chat pace (Relaxed/Normal/Brisk, `cgAI_glassos_pace`) lets players choose, `scripts/cadence-smoke.mjs` audits the real 1× feel (~3s typing beat, ~3.5s reaction), and every smoke runs at 8× via the same key. **M4/M5 surfaces (2026-09-11/12):** anthology screen transitions + the pace-aware anchor cold open (skippable boot card, reduced-motion safe), the glassOS notification center (tappable banners + a shade), gallery zoom, side-by-side artifact compare (`OSPhoto.series`; `scripts/compare-smoke.mjs`), voice-note play/pause, Notes search, link forward/share (`forwardMessage`: marked copies + Copy link, no story flags), and the deeper stage — time-of-day desk light (`timeMood`) plus a tilt-driven glass sheen (`shade-smoke.mjs`, `artifact-smoke.mjs`). **Design pass (2026-09-13):** an OpenDesign-driven review (headless daemon + `od lint` on rendered DOM + opencode v2 plan-mode assessment against the exported design system) produced 8 findings, all applied and verified (build, lint 0/0/0, hub+os smokes): phone chrome consumes the os-skin contract everywhere (home, banners, call overlay — no hardcoded whites/hex), `--os-faint` holds WCAG AA per skin, verdict tokens `--color-ok/warn/danger` plus `ink-200`/`--radius-card/-well`/`--shadow-card/-lift`/`--ease-glass`/`--text-2xs` (11px floor) landed in `@theme`, the hub got a ghost CTA + single-word status chips + `data-od-id` anchors, touch targets ≥44px, and the Contacts focus-ring striptease was reverted. Canonical design prose lives at `docs/design-system.md` (the OpenDesign `user:blackglass` package is an export of it; re-export = one POST + PATCH). The reusable headless workflow is the `opendesign-headless` skill at `~/.agents/skills/` (agent-agnostic: lint + tokens via daemon, assessment by whichever agent drives). All smoke/shot scripts now launch via system Chrome (`channel: 'chrome'`) — the ms-playwright cache is stale. See `play/blackglass-phones/VIDEO-WORKSHEET.md`, `research/ai-visual-workflows-2026.md`.
...
- Platform: `npm install`, `npm run dev` (dev server), `npm run build` (outputs `dist/`, deploys to GitHub Pages via `bash scripts/deploy.sh`). Actions is manual-only (`.github/workflows/ci.yml`).
