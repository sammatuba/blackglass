...
**Current milestone:** Phase 4 (glassOS & the premium pass) is **complete and deployed** (gh-pages; Actions deploys are blocked by a GitHub billing lock on the account — use `scripts/deploy.sh`). Shipped: glassOS runtime, 3 cases, 4 anthology anchors, Learn shelf, hero perf, 7 diegetic artifacts, Kitbitz debrief vignettes. **In flight — the video register pilot:** `dr-anita-still.mp4` (Veo i2v via the user's Google AI Pro, 10s/720p/9:16, 2.3MB) is committed, synced, and wired; runtime support (artifactVideo → real `<video>`, stills as posters, `OSTell.at` timestamps, videos out of PWA precache) shipped. **Next session, first thing:** run `node scripts/anita-video-shot.mjs` against `npm run preview` — it drives Maya→Bea→council→For You to the anita page but the last step fails: the dialog-dismiss loop clicks an exit control and bounces to the rack (see `/tmp/av-6-anita.png` state). Fix = dismiss only the narrative-moment dialog (its label button, e.g. "Don't put it down yet" — `div[role="dialog"]` whose text matches the moment, not the rack's dialogs), then assert `<video src*=dr-anita>` + poster. Then: deploy, and the Tito Mike clip is the same flow (`deepfake.ts`). See `play/blackglass-phones/VIDEO-WORKSHEET.md`, `research/ai-visual-workflows-2026.md`.
