...
const SERVICE_LABEL: Record<string, string> = { sms: 'Messages', viber: 'Viber', messenger: 'Messenger' }
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="flex items-baseline gap-2">
                <span className="truncate text-[14px] font-bold text-white/95">{t.name}</span>
                {last?.at && <span className="ml-auto shrink-0 text-[10px] text-white/45">{last.at}</span>}
              </span>
              <span className={`mt-0.5 line-clamp-1 block text-[12.5px] ${offered ? 'font-semibold text-white/90' : 'text-white/55'}`}>
                {typingHint(t.id, caseDef, os) ? (
                  <span className="italic text-emerald-300">typing…</span>
                ) : last ? (
                  `${last.from === 'you' ? 'You: ' : ''}${
                    last.kind === 'voice'
                      ? '🎙️ Voice message'
            ...
            {offered && <span className="h-2.5 w-2.5 shrink-0 rounded-full bg-emerald-400" aria-label="Your reply is being asked for" />}
          </button>
        ),
      )}
    </div>
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center gap-2.5 border-b border-white/10 px-4 py-2.5">
        ...
        <div className="min-w-0 flex-1">
          <div className="truncate text-[13.5px] font-bold text-white/95">{thread.name}</div>
      ...
      <div className="min-h-0 flex-1 space-y-2.5 overflow-y-auto px-3 py-4" aria-live="polite">
                ...
                className={`max-w-[82%] ${
                  m.kind === 'callcard'
                    ...
                    : `rounded-2xl px-3.5 py-2 text-[13.5px] leading-relaxed ${
                        out ? 'rounded-br-md bg-emerald-600/90 text-white' : 'rounded-bl-md bg-[#2a3550] text-white/95'
                      }`
                }`}
              >
                {!out && thread.members && m.at && (
                  <div className="mb-0.5 text-[10px] font-bold text-white/60">{thread.members[0]}</div>
                )}
                {m.kind === 'voice' ? (
                  ...
                  >
                    <span className="block overflow-hidden rounded-xl bg-[#1a2438]">
                  ...
                  <button
                    ...
                    className="block w-full text-left"
                  >
                    <span className="block overflow-hidden rounded-xl border border-white/10 bg-[#111a2e]">
                  ...
                  <div className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-center text-[12px] text-white/75">
                    {m.text}
                  </div>
                ) : (
                  m.text
                ...
                {m.at && m.kind !== 'callcard' && (
                  <div className={`mt-0.5 text-[9.5px] ${out ? 'text-right' : ''} text-white/45`}>{m.at}</div>
                )}
              </div>
            </div>
          )
        ...
        {typing && (
          <div className="flex justify-start">
            <div className="flex items-center gap-1 rounded-2xl rounded-bl-md bg-[#2a3550] px-4 py-3" aria-label={`${thread.name} is typing`}>
              {[0, 1, 2].map((i) => (
                <span
                  key={i}
          ...
          <div className="space-y-2" role="group" aria-label="Suggested replies">
            {offered.map((r) => (
              <button
                ...
                className="block w-full rounded-2xl border border-emerald-400/40 bg-emerald-500/10 px-4 py-2.5 text-left transition-colors hover:bg-emerald-500/20"
              >
                <span className="block text-[13.5px] font-semibold text-white/95">{r.label}</span>
                {r.sub && <span className="mt-0.5 block text-[11px] leading-snug text-white/55">{r.sub}</span>}
              </button>
            ))}
          </div>
        ) : (
          <div className="flex items-center gap-2 rounded-full bg-white/8 px-4 py-2.5 text-[12.5px] text-white/35">
  ...
  return (
    <div>
      <button type="button" onClick={start} className="flex items-center gap-2.5" aria-label={`Play voice message, ${secs} seconds`}>
        <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-white/15 text-[11px]">
          {playing ? '❚❚' : '▶'}
        </span>
        <span className="flex h-6 items-center gap-[2px]" aria-hidden="true">
          {bars.map((h, i) => (
            <span
              key={i}
              className={`w-[2.5px] rounded-full transition-colors ${i / bars.length < progress ? 'bg-emerald-300' : 'bg-white/45'}`}
      ...
      {note && progress >= 1 && (
        <p className="mt-1.5 border-t border-white/10 pt-1.5 text-[11px] italic leading-snug opacity-75">{note}</p>
      )}
    </div>
  )
}
    ...
    <div className="h-full overflow-y-auto px-3 pb-8">
      ...
      {open && (
        <div className="absolute inset-0 z-50 flex animate-fadein flex-col bg-black/95 p-4 pt-10" role="dialog" aria-label={`Photo: ${open.title}`}>
          <button type="button" onClick={() => setOpenId(null)} className="absolute left-4 top-3 rounded-full bg-white/10 px-3 py-1.5 text-xs text-white/80" aria-label="Close photo">
            ‹ Back
          </button>
          <div className="flex min-h-0 flex-1 flex-col justify-center">
            ...
            {open.tells && open.tells.length > 0 && (
              <div className="mt-3 space-y-2 overflow-y-auto">
                <p className="text-[10px] font-bold tracking-[0.2em] text-emerald-300 uppercase">Look again</p>
                {open.tells.map((t) => (
                  <div key={t.label} className="rounded-xl border border-white/10 bg-white/5 p-3">
                    <div className="text-[12px] font-bold text-white/90">🔍 {t.label}</div>
                    <p className="mt-0.5 text-[11.5px] leading-relaxed text-white/65">{t.detail}</p>
                  </div>
                ))}
              </div>
            )}
...
export function PhoneApp({
  ...
  return (
    <div className="h-full overflow-y-auto pb-8">
      <h3 className="px-4 pt-2 text-[10px] font-bold tracking-[0.2em] text-white/45 uppercase">Voicemail</h3>
      {vms.length === 0 ? (
        <p className="px-4 py-6 text-xs text-white/40">No voicemails.</p>
      ) : (
        vms.map((v) => (
          <div key={v.id} className="px-3">
            <button
              ...
              className="mt-2 flex w-full items-center gap-3 rounded-2xl border border-white/8 bg-white/5 px-3.5 py-3 text-left transition-colors hover:bg-white/10"
            >
              <span className="grid h-9 w-9 place-items-center rounded-full bg-white/10 text-sm" aria-hidden="true">
                🎙️
              </span>
              <span className="min-w-0 flex-1">
                <span className="block truncate text-[13.5px] font-bold text-white/95">{v.from}</span>
                <span className="block truncate text-[11px] text-white/50">
                  {v.number} · {v.at} · 0:{String(v.secs).padStart(2, '0')}
                </span>
              </span>
              <span className="text-white/40">{openId === v.id ? '⌃' : '›'}</span>
            </button>
            {openId === v.id && (
              <div className="mt-1 space-y-1.5 rounded-xl border border-white/8 bg-black/30 p-3" aria-live="polite">
                <p className="text-[10px] font-bold tracking-[0.2em] text-white/45 uppercase">Transcript</p>
                {v.transcript.map((line, i) => (
                  <p key={i} className="text-[12.5px] leading-relaxed text-white/80">
                    {line}
                  </p>
                ))}
              </div>
      ...
      {(caseDef.recents?.length ?? 0) > 0 && (
        <>
          <h3 className="mt-5 px-4 text-[10px] font-bold tracking-[0.2em] text-white/45 uppercase">Recents</h3>
          {(caseDef.recents ?? []).map((r, i) => (
            <div key={i} className="flex items-center gap-3 px-5 py-2.5">
              <span className={`text-sm ${r.missed ? 'text-red-400' : 'text-white/60'}`} aria-hidden="true">
                {r.missed ? '↙' : r.outgoing ? '↗' : '📞'}
              </span>
              <span className="min-w-0 flex-1">
                <span className={`block truncate text-[13px] font-semibold ${r.missed ? 'text-red-300' : 'text-white/90'}`}>{r.name}</span>
                <span className="block text-[10.5px] text-white/45">{r.number}</span>
              </span>
              <span className="text-[10.5px] text-white/40">{r.at}</span>
            </div>
          ))}
        </>
      )}
...
export function BrowserApp({
  ...
  if (!page) {
    return (
      <div className="flex h-full flex-col items-center px-6 pt-10 text-center">
        ...
        <p className="mt-2 text-sm font-semibold text-white/70">No page open</p>
        {visited.length > 0 ? (
          <>
            <p className="mt-1 text-xs text-white/45">Recently opened from messages:</p>
            <div className="mt-3 w-full space-y-2">
              {visited.map((p) => (
                <button key={p.id} type="button" onClick={() => onOpenPage(p.id)} className="block w-full rounded-xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-left text-left">
                  <span className="block truncate text-[12.5px] font-semibold text-white/90">{p.title}</span>
                  <span className="block truncate text-[10.5px] text-white/45">{p.url}</span>
                </button>
              ))}
            </div>
          </>
        ) : (
          <p className="mt-1 text-xs text-white/45">Links you receive in messages open here.</p>
        )}
      </div>
    )
  }
  ...
  return (
    <div className="flex h-full flex-col">
      {/* URL bar */}
      <div className="mx-3 flex items-center gap-2 rounded-full border border-white/10 bg-black/40 px-3.5 py-2">
        <span className="text-[10px]" aria-hidden="true">🔒</span>
        <span className="min-w-0 flex-1 truncate text-[11.5px] text-white/80">{page.url}</span>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-6 pt-3">
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-white text-[#1a2233]">
          {/* page header */}
  ...
  return (
    <div className="flex h-full flex-col px-3">
      <input
        ...
        className="mx-1 mb-2 rounded-full border border-white/10 bg-black/30 px-4 py-2 text-[13px] text-white/90 placeholder:text-white/35 focus:outline-none"
      />
      <div className="min-h-0 flex-1 overflow-y-auto pb-6">
        {list.map((c) => (
          <button
            ...
            className="mt-1 flex w-full items-center gap-3 rounded-xl px-2 py-2 text-left transition-colors hover:bg-white/5"
          >
            <Avatar name={c.name} hue={(c.name.length * 57) % 360} size="sm" />
            <span className="min-w-0 flex-1">
              <span className="block truncate text-[13.5px] font-bold text-white/95">{c.name}</span>
              <span className="block truncate text-[11px] text-white/50">{c.number}</span>
            </span>
            {c.relation && <span className="shrink-0 text-[10px] text-white/40">{c.relation}</span>}
          </button>
        ))}
        {list.length === 0 && <p className="px-3 py-6 text-center text-xs text-white/40">No contacts match.</p>}
        {open && (
          <div className="mt-3 rounded-2xl border border-white/10 bg-white/5 p-4" aria-live="polite">
            <div className="text-[15px] font-bold text-white/95">{open.name}</div>
            <div className="mt-0.5 text-[12.5px] text-white/70">{open.number}</div>
            {open.relation && <div className="mt-0.5 text-[11px] text-white/45">{open.relation}</div>}
            {open.evidence && (
              <p className="mt-3 rounded-xl border border-emerald-400/25 bg-emerald-500/10 p-3 text-[12px] leading-relaxed text-emerald-100">
  ...
  return (
    <div className="h-full overflow-y-auto px-4 pb-8">
      <div className="mt-1 rounded-2xl border border-amber-300/25 bg-amber-400/10 p-4">
        ...
        <p className="mt-1 text-[12.5px] leading-relaxed text-white/80">{caseDef.blurb}</p>
      </div>

      <h3 className="mt-5 text-[10px] font-bold tracking-[0.2em] text-white/45 uppercase">Evidence collected · {evidence.length}</h3>
      {evidence.length === 0 ? (
        ...
        <ul className="mt-2 space-y-1.5">
          {evidence.map((e) => (
            <li key={e.id} className="flex gap-2 rounded-xl border border-white/8 bg-white/5 px-3 py-2 text-[12.5px] leading-relaxed text-white/85">
              <span className="text-emerald-300" aria-hidden="true">✓</span> {e.label}
            </li>
          ))}
        </ul>
      )}

      <h3 className="mt-5 text-[10px] font-bold tracking-[0.2em] text-white/45 uppercase">Notes</h3>
      <div className="mt-2 space-y-2 pb-2">
        {caseDef.notes.map((n) => (
          <div key={n.title} className="rounded-2xl border border-white/8 bg-white/5 p-3.5">
            <div className="text-[12.5px] font-bold text-white/90">{n.title}</div>
...
export function SettingsApp({
  ...
  return (
    <div className="h-full overflow-y-auto px-4 pb-8">
      <h3 className="mt-1 text-[10px] font-bold tracking-[0.2em] text-white/45 uppercase">Display</h3>
      <div className="mt-2 rounded-2xl border border-white/8 bg-white/5 p-4">
        <label htmlFor="brightness" className="text-[12.5px] font-semibold text-white/85">
          Brightness
        </label>
        <input
          id="brightness"
      ...
      <h3 className="mt-4 text-[10px] font-bold tracking-[0.2em] text-white/45 uppercase">Sound</h3>
      <button
        type="button"
        role="switch"
        aria-checked={!muted}
        ...
        className="mt-2 flex w-full items-center justify-between rounded-2xl border border-white/8 bg-white/5 p-4"
      >
        <span className="text-[12.5px] font-semibold text-white/85">Interface sounds</span>
        <span className={`relative h-6 w-11 rounded-full transition-colors ${!muted ? 'bg-emerald-500' : 'bg-white/20'}`}>
          <span className={`absolute top-0.5 h-5 w-5 rounded-full bg-white transition-all ${!muted ? 'left-[22px]' : 'left-0.5'}`} />
        </span>
      </button>

      <h3 className="mt-4 text-[10px] font-bold tracking-[0.2em] text-white/45 uppercase">About this phone</h3>
      <div className="mt-2 space-y-1 rounded-2xl border border-white/8 bg-white/5 p-4 text-[12px] text-white/65">
        ...
        <p className="text-white/40">{caseDef.phone.day} · battery {caseDef.phone.battery}%</p>
      </div>
    </div>
  )
}
