#!/usr/bin/env python3
"""Extract narrative text from Grotto's Unity bundles.

Grotto is Unity 2020.2 Mono. Narrative lives in **MonoBehaviour
ScriptableObjects** — `ScriptableEvent`, `ScriptableVisit`, `NarratorEvent`,
`ScriptableStarCombo`, `ConstellationDescription`, `StarText`, `ScriptableSegment`,
`ChoiceDetector`. TextAssets in this build are mostly Spine skeleton animation
data and aren't useful.

MonoBehaviour typetrees are reconstructed from Managed/*.dll via UnityPy's
TypeTreeGenerator (same pattern as Sir Brante).
"""
