#!/usr/bin/env python3
"""Extract narrative text from Grizzly Man's Unity bundles.

Grizzly Man is Unity 2020.3 Mono, driven by the **Naninovel** visual-novel
engine. Narrative is stored as:
  - `Naninovel.Script` MonoBehaviours (~305) — each holds a `ScriptText`
    string that is the full NaniScript source for one scene
  - `Naninovel.ManagedTextProvider` MonoBehaviours — key→value strings by
    category (UI, credits, achievements, etc.)
  - TextAssets contain the backing Managed Text payloads (`.Key: Value` lines)

Post-process with `resolve.py` to pull every `Naninovel.Script` into its own
readable `.nani` file.
"""
...
NARRATIVE_CLASS_HINTS = (
    "naninovel", "script", "playscript", "managedtext",
