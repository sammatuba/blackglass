...
4. **Presentation polish** ◐ — screen transitions, the glassOS notification center, human chat cadence, and the deeper stage (time-of-day desk light + tilt-driven glass sheen) shipped; the anthology boot sequence is still backlog (M4).
5. **Interaction depth** ◐ — gallery zoom, voice-note play/pause, Notes search, and link forward/share (Forward with a thread picker + Copy link, marked copies, no story flags) shipped; side-by-side artifact compare still backlog (M5).
