...
5. **Showpiece polish** — Kitbitz CC0 scene vignettes in case debriefs (`docs/ASSETS.md`); hero perf groundwork done: the Three.js hero chunk is excluded from the PWA precache (capability-gated load, degrades to the CSS gradient — first-visit cache 2,417 KB → 1,554 KB), hub smoke added (`scripts/hub-smoke.mjs`). Further hero work (TSL/RenderPipeline refinement) optional.
6. **Diegetic artifacts — pipeline decision pending.** The Gemini-image plan is superseded by options from `research/ai-visual-workflows-2026.md`: free local generation with Apache/MIT open models (Qwen-Image for text-heavy artifacts — headlines/lower-thirds), stills as poster-frame fallbacks, and a recommended **AI-video pilot** (Wan 2.2, Apache, on Kaggle's free T4) for the video-native artifacts (Dr. Anita clip; planted tells gain time ranges). Needs a user decision on tooling + an `docs/ASSETS.md` rule-3 wording amendment ("Gemini pipeline" → "open-generation pipeline"). The old `GEMINI_API_KEY` blocker is no longer on the critical path.
